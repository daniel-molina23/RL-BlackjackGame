...
class MCTSPlayer(Player):
    ...
    def __init__(self, name, deck):
        ...
        self.bet = 2
        self.deck = deck
    def get_action(self, cards, actions, dealer_cards): ######## THIS IS WHAT WE EDIT!!!!!!!
        ...
        for i in range(MCTS_N):
            ...
            # print("-------------------------------New Simulation! Round # ", i + 1, "-----------------------------------") # DELETE
        ...
        act = p.getBestAction() # extracts the best action based on the tree constructed!
        
        # Make sure we also record our own bet in case we double down (!)
        if act == Action.DOUBLE_DOWN: # THIS ONE HAS *4, pass by ref
            self.bet *= 2
        return act
    def reset(self):
        self.bet = 2
...
class RolloutPlayer(Player):
    ...
    def __init__(self, name, deck):
        ...
        self.actions = []
        self.deck = deck
        self.root = Node(None) # root node never changes, unless we have a new initial state we want to simulate the best action for
        self.listOfActions = [] # list of actions taken for backtracking! for one single simulation!
        self.curr = self.root # start at root, and signify leaf node for the rest of iterations!
        self.split_flag = False # initially, only changed if split is possible for 4th action SPLIT
        self.iteration = 0 # starts at 0 to index the self.listOfActions list

    def get_action(self, cards, actions, dealer_cards): # Not being used
    ...
    def monte_carlo_tree_search(self, game, cards, dealer_cards, bet):
        leafNode = self.traverse(self.root) # traverses tree until leaf Node is found. Note: Doesn't change which cards you have
        self.curr = leafNode
        simulation_reward = game.continue_round(cards, dealer_cards, bet) # simulate!
        self.backtrackExpectations(self.curr, simulation_reward) # every expected value in the tree gets updated

    def getBest_MCTS_Action(self, cards): #used in play() method
        if(len(self.listOfActions)>1 and self.listOfActions[0] == Action.SPLIT and self.split_flag): # CHANGE: split_flag may be redundant
            temp = None
            if(len(cards)==1): # means we start fresh, a new split action
                self.iteration = 1 # index after the SPLIT action
            if(self.iteration < len(self.listOfActions)): # ensure proper indexing
                temp = self.listOfActions[self.iteration]
            self.iteration += 1
            return temp
        elif(self.iteration < len(self.listOfActions) and self.iteration > -1): # CHANGE: Find a way to reset iteration after every round!
            temp = self.listOfActions[self.iteration]
            self.iteration += 1
            return temp

        return None # if none of the above is valid

    def isLeafNode(self):
        return len(self.curr.children) == 0 or self.curr == self.root # CHANGE: not sure if it should be (len) or (len-1)

    def best_UCB(self, node):
        # formula: UCB = average + 2 * sqrt(ln(parentVisitations)/currentNode visitations)
        maxChild = None
        maxUCB = -100000
    ...
    def notTerminalState(self, node): # must be the root, Hit action, or split action!
        return (node.actionType == Action.HIT.value or node.actionType == Action.SPLIT.value or self.root == node)

    def traverse(self, root):
        currNode = root
        while(len(currNode.children)!=0 and self.notTerminalState(currNode)): # node has children and node not terminal state
            currNode = self.best_UCB(currNode) # find best possibility!
            self.listOfActions.append(Action(currNode.actionType))
        # listOfActions has been successfully modified!
        return currNode
